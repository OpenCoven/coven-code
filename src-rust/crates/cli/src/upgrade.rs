// upgrade.rs — `coven-code upgrade` subcommand.
//
...
// so that an `upgrade` from inside Coven Code feels identical to a fresh install.
//
...
pub async fn run_upgrade(args: &[String]) -> Result<()> {
    ...
    println!("Run `coven-code --version` in a new shell to verify.");
    Ok(())
}

fn print_help() {
    println!(
        "Usage: coven-code upgrade [options]\n\n\
         Options:\n\
           -v, --version <v>   Install a specific version (default: latest)\n\
           -f, --force         Reinstall even if already up to date\n\
           -h, --help          Show this help\n\n\
         Downloads the latest coven-code release from GitHub and replaces this\n\
         binary in place. Settings in ~/.coven-code are preserved."
    );
}
...
async fn fetch_latest_version() -> Result<String> {
    ...
    let resp = match req.send().await {
        ...
        Err(err) => {
            return fetch_latest_version_from_redirect()
                .await
                .with_context(|| format!("GitHub API request failed for {url}: {err}"));
        }
    };
    if !resp.status().is_success() {
        ...
        return fetch_latest_version_from_redirect()
            .await
            .with_context(|| format!("GitHub API returned {status} for {url}"));
    }
...
async fn fetch_latest_version_from_redirect() -> Result<String> {
    ...
    release_version_from_location(location)
        .ok_or_else(|| anyhow!("could not parse release version from redirect: {location}"))
}
...
fn swap_binary(current: &Path, new: &Path) -> Result<()> {
    #[cfg(target_os = "windows")]
    {
        ...
        if let Err(e) = std::fs::copy(new, current) {
            // Try to roll back the rename so the user isn't left without coven-code.
...
mod tests {
    ...
    fn parses_latest_release_redirect_locations() {
        ...
        assert_eq!(
            release_version_from_location(
                "https://github.com/OpenCoven/coven-code/releases/tag/v0.2.0?expanded=true"
            )
            .as_deref(),
            Some("0.2.0")
        );
        assert_eq!(
            release_version_from_location("https://github.com/OpenCoven/coven-code/releases"),
            None
        );
    }
}
