...
pub const COORDINATOR_ENV_VAR: &str = "COVEN_CODE_COORDINATOR_MODE";
...
/// Tools that workers are allowed to use in simple mode (COVEN_CODE_SIMPLE=1).
...
pub fn filter_tools_for_mode(
    ...
    match mode {
        AgentMode::Coordinator | AgentMode::Normal => {
            tools.iter().map(|tool| tool.as_ref()).collect()
        }
        AgentMode::Worker => tools
            .iter()
            .map(|tool| tool.as_ref())
            .filter(|tool| !COORDINATOR_ONLY_TOOLS.contains(&tool.name()))
            .collect(),
    }
}
