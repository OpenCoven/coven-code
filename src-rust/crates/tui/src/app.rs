...
impl App {
    ...
    pub fn signal_bg_task_completion(
        ...
        if toast_enabled {
            ...
            let cmd_suffix = if info.command.chars().count() > 48 {
                "…"
            } else {
                ""
            };
    ...
    /// Take the next key event saved by `try_detect_paste_burst` when
...
"#,
    ...
    fn bg_task_failure_pushes_error_toast_no_autoexpire() {
        ...
        assert!(
            n.expires_at.is_none(),
            "errors should stay until acknowledged"
        );
    }
