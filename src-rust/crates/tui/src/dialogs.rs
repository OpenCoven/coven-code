...
pub fn render_permission_dialog(frame: &mut Frame, pr: &PermissionRequest, area: Rect) {
    ...
    let (border_color, title_text) = match &pr.kind {
        ...
        PermissionDialogKind::FileRead { .. } => {
            (Color::Rgb(167, 139, 250), " File Read Permission ")
        }
...
fn build_dialog_content(dialog: &ToolPermissionDialog) -> (&'static str, Vec<String>) {
    match &dialog.kind {
        ...
        ToolPermissionKind::AskUser { question, choices } => ("Familiar Question", {
...
mod tests {
    ...
    fn word_wrap_mixed_short_and_long_tokens() {
        use unicode_width::UnicodeWidthStr;
        // The realistic shape that broke coven-code dialogs: a normal command
        // followed by a path longer than the column budget.
        let cmd = "git diff 'X:\\Bigger-Projects\\Coven Code\\very\\deep\\nested\\path.rs'";
