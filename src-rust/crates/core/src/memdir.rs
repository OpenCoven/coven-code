...
pub fn scan_memory_dir(dir: &Path) -> Vec<MemoryFileMeta> {
    ...
    // Sort newest-first.
    files.sort_by_key(|file| std::cmp::Reverse(file.modified_secs));
...
/// 2. `<COVEN_CODE_REMOTE_MEMORY_DIR>/projects/<sanitized-root>/memory/`
///    when `COVEN_CODE_REMOTE_MEMORY_DIR` is set.
/// 3. `~/.coven-code/projects/<sanitized-root>/memory/` (default).
...
/// 1. `COVEN_CODE_DISABLE_AUTO_MEMORY` — truthy → OFF, falsy (but defined) → ON.
/// 2. `COVEN_CODE_SIMPLE` (--bare) → OFF.
/// 3. Remote mode without `COVEN_CODE_REMOTE_MEMORY_DIR` → OFF.
...
pub fn is_auto_memory_enabled(settings_enabled: Option<bool>) -> bool {
    if let Ok(val) = std::env::var("COVEN_CODE_DISABLE_AUTO_MEMORY") {
    ...
    if std::env::var("COVEN_CODE_SIMPLE").is_ok() {
        return false;
    }

    if std::env::var("COVEN_CODE_REMOTE").is_ok()
        && std::env::var("COVEN_CODE_REMOTE_MEMORY_DIR").is_err()
    {
        return false;
    }
