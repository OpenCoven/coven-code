...
impl StashStore {
    ...
    pub fn add_attachment(
        ...
        // Key the stored copy on the full id so two attachments can never
        // collide on a shared directory, even with equal filenames.
        let dest_dir = attachments_dir.join(&id);
        std::fs::create_dir_all(&dest_dir).map_err(|e| StashError::Io(e.to_string()))?;
        let dest = dest_dir.join(&filename);
        if dest.exists() {
            return Err(StashError::Io(format!(
                "stored attachment path already exists: {}",
                dest.display()
            )));
        }
    ...
    /// Find a single item by full id or unique id prefix, optionally
    /// restricted to one kind so `/link` cannot resolve an attachment id
    /// (and vice versa).
    pub fn get(&self, id_prefix: &str, kind: Option<StashKind>) -> Result<StashItem, StashError> {
...
mod tests {
    ...
    fn add_attachment_copies_file_and_remove_cleans_up() {
        ...
        let removed = store
            .remove(item.short_id(), Some(StashKind::Attachment))
            .unwrap();
    ...
    fn get_by_prefix_and_ambiguity() {
        ...
        assert_eq!(store.get(&a.id[..8], None).unwrap().id, a.id);
        assert!(matches!(
            store.get("zzzzzz", None),
            Err(StashError::NotFound(_))
        ));
        // Empty prefix matches everything → ambiguous.
        assert!(matches!(store.get("", None), Err(StashError::Ambiguous(_))));
    }

    #[test]
    fn kind_scoped_lookup_cannot_cross_kinds() {
        let (dir, store) = temp_store();
        let link = store
            .add_link("https://a.example", None, None, &[], None, None)
            .unwrap();
        ...
        // A link lookup must not resolve an attachment id, and removing via
        // the wrong kind must not delete the stored file.
        assert!(matches!(
            store.get(&attachment.id, Some(StashKind::Link)),
            Err(StashError::NotFound(_))
        ));
        assert!(matches!(
            store.remove(&attachment.id, Some(StashKind::Link)),
            Err(StashError::NotFound(_))
        ));
        assert!(PathBuf::from(&attachment.value).exists());

        assert!(matches!(
            store.get(&link.id, Some(StashKind::Attachment)),
            Err(StashError::NotFound(_))
        ));
        assert_eq!(
            store
                .get(&attachment.id, Some(StashKind::Attachment))
                .unwrap()
                .id,
            attachment.id
        );
